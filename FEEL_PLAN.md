...
| 34 | The BFG charge look (RS_Ballistics 9474275's RSB_Barrel.Charge), and a new map no longer orphans a gun prop (the build lane doomwork-5e's go, 2026-09-14) | **COMPILED 2026-09-14 (uzdxrema-11)** on exe 09-14 10:02, installed 11:24:35 (script parsing took 785.68 ms). Not yet played.<br>• **`WM_Rig.ChargeLook`:** every tic after BarrelSmoke, while the gun is drawn, resolved and not stowed, and its hand's psprite is in WM_Gun's Charge state, it calls `RSB_Barrel.Charge(gunItem, FlashProfileOrDefault, MuzzleWorld, BarrelWorld, fraction)`.<br>• **The fraction:** `(level.maptime - WM_Gun.chargeBeganTic) / ChargeTicsEach`. WM_ChargeWait stamps `chargeBeganTic` on the same tic it sets the charge's length, so the look can't drift from the real wind-up.<br>• **Netplay:** presentation only, local, no RNG.<br>• **`WM_System.WorldLoaded`:** `if (rig.prop) rig.prop.Destroy();` before forgetting the prop. PlayerSpawned fires before WorldLoaded, so a PutGunInHand in that window (36794c6) would otherwise leave a second gun drawn in the hand. No caller does that today. |
| 35 | `WM_Gun.SawPuff "class"`: a saw names its own cut (the ballistics lane's chainsaw looks; the build lane's go, 2026-09-14) | **COMPILED** in the same build as row 34. Not yet played.<br>• **`WM_Gun.SawPuffActor()`:** finds the class by name. Unset, or a name that is no actor (logged once), gives RS_Ballistics' RSB_SawPuff; without that, the engine's BulletPuff. A_Saw in WM_TryFire uses it instead of the hard-coded "RSB_SawPuff".<br>• **Defaults:** no saw changes until a class sets it. The weapons lane sets WM_ChainsawHeavy to "RSB_SawPuffHeavy".<br>• **Netplay:** read off the class's Default block, alike on every machine; no RNG.<br>• **The exhaust point** (a card key plus RSB_Barrel.Engine) is PARKED. |
| 36 | Measured grabs: the ESTIMATE grab points and sizes measured off the meshes, into RS_VR_Weapons/WMCARD.txt (the build lane's go, 2026-09-14) | **COMPILED** and installed in the weapons lane's staged check: 769.12 ms on exe 10:02, installed 11:28:42; card_lint shows 0 issues. Not yet played.<br>• **The edits:** 16, by `_pending/MEASURED_GRABS/apply_grabs.py` (exact-match).<br>• **Measured:** the Super Shotgun's and Double Barrel's barrels grabsize; the support grabs on both plasmas, the RPG, both BFGs and the Machine Gun; the Heavy BFG's cell; the Machine Gun's launcher tube.<br>• **Radius 3.0** is kept and now documented as the house reach tolerance, not a mesh size.<br>• **Still ESTIMATE:** the Bullpup's port and gate (the mesh has no port) and the saws' support points and radii.<br>• **Netplay:** card data only, read alike on every machine. |
| 37 | THE_CARD_PLAN step 2: the WMSHEET reader, sheets applied to cards and guns, and the `wm_card` printout (the owner's go, 2026-09-14; key table from the weapons lane) | **COMPILED** 798.88 ms on exe 09-14 13:12, installed 13:20:43. No sheet exists yet, so no gun changes.<br>• **`zscript/wm/sheet.zs`:** `gun "<class>"` … `end` with every WM_Gun shot property in lower case, plus `recoilprofile` / `altrecoilprofile` (stored only, until the recoil hookup), `capacity`, `firesfrom`, `firesound`, and a `barrel <id>` sub-block for a second barrel's shot. Unknown and card-only keys are refused with the line and the reason.<br>• **Card keys** land on the card in `WM_System.LoadCards`, before `WM_Parser.Finish`, so Finish checks the card as the sheet leaves it.<br>• **Shot keys:** `WM_Gun.TakeSheet` resets every shot field to the class Default, then lays the sheet over it. It runs at WorldLoaded for every WM_Gun and in `WM_Gun.PostBeginPlay`, never on a rebind (console player only).<br>• **`wm_card <class> / all / check`** (KEYCONF, `netevent wm_card:<arg>`) prints on the sender's machine only. `check` lists every sheet value that differs from the class or card.<br>• **Saves:** a set saved before sheets is read again (`WM_CardSet.sheetsRead`).<br>• **Netplay:** data on every machine, applied in the playsim alike everywhere; no RNG, no consoleplayer, no cvar. |
| 38 | Borrowed model cards: a weapon sheet's `model = <model card id>`, and a `class` … `end` block the reader skips (the owner: any Weapon Card can use any Model Card; the weapons lane's class writer; 2026-09-14) | **COMPILED** 786.82 ms on exe 09-14 13:12, installed 13:44:15. No sheet names a model yet, so no gun changes.<br>• **`sheet.zs BorrowModels`:** before ApplyToCards and Finish, a gun whose sheet names a model gets ITS OWN COPY of that card. The copy is read again from the card's own WMCARD lump (`WM_Card.sourceLump`), renamed to the gun's class, and takes the hand from the gun's `+WEAPON.OFFHANDWEAPON`. So no live part state is shared with the model's own gun (NETPLAY_SPEC problem 7). A model that is itself a copy reads from its own model's lump.<br>• **Refused, with an error:** a gun with its own card AND a model line (it keeps its own), and a model id no card has.<br>• **`class` … `end`** inside `gun` is skipped whole; it is build-time data for RS_VR_Weapons' class writer.<br>• **`wm_card`** shows "(this gun's own copy)".<br>• **Saves:** `WM_CardSet.modelsRead` re-reads a set saved before this.<br>• **Netplay:** load-time data on every machine; no RNG, no consoleplayer. |
| 39 | Gameplay recoil on the guns (RS_Ballistics' RSB_Recoil, `_staged/RECOIL_PLAN.md`; the owner: "on for gameplay effects, off for no recoil or anything like that", ON by default, 2026-09-14) | **COMPILED** 765.61 ms on exe 09-14 13:12, installed 13:50:09. Not yet played.<br>• **`WM_Gun.RecoilStep`** runs once a shot in WM_TryFire, after the dead-on test and before the pellets (not a saw or a rail). `RSB_Recoil.Step` moves the gun's kick (saved fields `recoilPitch`, `recoilYaw`, `recoilShotTic`, `recoilRunShot`). Its bloom widens the spread, and `RSB_Recoil.Turn` turns each round before ScatterShot.<br>• **WM_TryAltFire** does the same on the sheet's `altrecoilprofile`, on the same kick.<br>• **`WM_Rig.RecoilLook`** runs every drawn tic: each new shot slides the prop back along its barrel by ViewJolt's `back` and eases it home, through `FollowHandOfs` (-X is back in either hand).<br>• **The switch:** `sv_rsb_recoil` off gives zero kick, zero spread and no jolt (Step, View and ViewJolt all return 0; RS_Ballistics 7ef2606).<br>• **Waiting:** the climb and roll the drawn gun should carry need the engine's `FollowHandRot` (proposed to the build lane, approved by the owner).<br>• **Netplay:** Step and Turn run in the fire action on every machine, from the map clock, the shot count and the owner's crouch and speed; no RNG, never consoleplayer. The look is local only. |
| 40 | The saw exhaust (RS_Ballistics' RSB_Exhaust, e2182cf; the owner's "fill all", 2026-09-14), plus a fix to row 39's slide axis | **COMPILED** 756.56 ms on exe 09-14 13:12, installed 14:02:47. Not yet played.<br>• **Card keys** `exhaustport = x, y, z` / `exhaustdir = x, y, z` (card.zs, parser.zs; model space, like ejectport). A sheet that names them is told they belong on the card.<br>• **`WM_Rig.ExhaustLook`** runs every tic the engine runs (EngineIdle's own rule: a caught ripcord, or an idle-sound gun with no start verb), drawn, not stowed, owner alive. It calls RSB_Exhaust.Run with the gun's flash profile, the port and way in world space, and throttle 1 while the trigger is held, 0 at idle.<br>• **Ports** (RS_VR_Weapons WMCARD.12_chainsaw):<br>&nbsp;&nbsp;– Heavy Chainsaw: MEASURED, between its twin pipes' rear openings, blowing back along them.<br>&nbsp;&nbsp;– Chainsaw and Longbar: ESTIMATE of which feature (no muffler is modelled), MEASURED on the engine's front face above or below the bar, blowing forward.<br>• **Waiting:** the puffs need the saws' flash profiles `chainsaw` / `chainsaw_heavy` in their sheets (the weapons lane).<br>• **ROW 39 FIX:** FollowHandOfs lands in the HAND's frame (X right, mirrored in the off hand; Y back; Z up; models.cpp step 4), so the recoil slide is now `(0, slide, 0)`. It had been on X, sideways.<br>• **Netplay:** presentation on this machine only; no RNG, nothing read back. |
| 41 | The recoil TILT (the engine's FollowHandRot, 15908db746), and RS_WeaponAmmoService for the weapon HUD (RS_VRPanels, from Ermac's Rusted Legacy; the HUD panels lane's request via the build lane), 2026-09-14 | **COMPILED** 1006.84 ms on exe 09-14 14:43. NOT installed: the owner was testing, so the build lane installs on its word. Not yet played.<br>• **The tilt:** `WM_Rig.RecoilLook` now also writes `prop.FollowHandRot = (kickYaw, -(kickPitch + rise × share), roll × share) × RECOIL_TILT_SIGN`. The kick comes from RSB_Recoil.View (0 with recoil off); rise and roll from ViewJolt, easing home with the slide. Zeroed when stowed or unbound.<br>&nbsp;&nbsp;– Signs, from the build lane's reading: + yaw turns the muzzle left, + pitch drops it, + roll tips the top right.<br>&nbsp;&nbsp;– `RECOIL_TILT_SIGN` (1.0) flips all three after a headset look.<br>• **RS_WeaponAmmoService** (zscript/wm/ammoservice.zs) answers GetInt, GetIntUI and GetIntData:<br>&nbsp;&nbsp;– "rounds": every live round in the gun's own main-barrel stores, a detached magazine not counted; a reserve-fed gun gives its reserve. -1 for not WM, fires nothing, or never bound here.<br>&nbsp;&nbsp;– "reserve": AmmoType1 carried. -1 for none, or a reserve-fed gun.<br>&nbsp;&nbsp;– "state": bits 1 chamber empty, 2 magazine out, 4 action locked.<br>&nbsp;&nbsp;– So it needs no card, `WM_Ammo.firesFrom` and `WM_Store.barrelStore` are set at every `WM_Rig.Bind`.<br>• **Netplay:** reads only, presentation only, local. The tilt draws locally from the gun's saved kick fields. |
| 42 | The Chaingun's SPIN-UP with an alt-button pre-spin, and a HAPTIC LOAD GUIDE (the owner's picks by multiple choice, 2026-09-15, for the Force Unleashed guns' swap: the Chaingun on RS_GH_Minigun with no reload, the Rocket Launcher loaded one rocket at a time into the front of its tube) | **COMPILED** 779.12 ms on exe 09-14 23:54, INSTALLED 09-15 01:06:56 on the owner's word ("install it all"). Not yet played.<br>• **`WM_Gun.SpinUpTics` / `SpinDownTics`** (sheet keys `spinuptics` / `spindowntics`, 0 to 350): barrels that must turn N tics before a burst.<br>&nbsp;&nbsp;– `SpinStep`, from DoEffect every tic, keeps a whole-number `spinCount`: up while this hand's trigger is down, or its second button when the card puts no barrel there; down otherwise. Full speed is up-tics × down-tics, so both directions are whole steps. SpinDownTics unset is twice SpinUpTics.<br>&nbsp;&nbsp;– `WM_Charge` first holds the shot in the new `SpinUp` state (`WM_SpinWait`, a tic at a time) until the barrels are at full speed. Letting go of the trigger, or a switch pending for this hand, goes back to Ready with nothing fired. A pull on spun barrels fires at once, so holding the second button keeps the gun ready.<br>&nbsp;&nbsp;– `WM_Rig.Spin`: a `spin = trigger` part on such a gun shows `SpinFraction()`, so the barrels reach full speed on the tic the gun may fire. The card's own spinup / spindown still drive every other spinning part.<br>&nbsp;&nbsp;– Default 0: no gun changes until a sheet says so. Suggested for WM_Chaingun and WM_VP_Chaingun: `spinuptics = 10`, `spindowntics = 20`. The weapons lane adds them once this is installed; an older pack refuses an unknown sheet key and with it the whole sheet.<br>• **The load guide** (`WM_System.LoadBuzz`, from Carry, with `WM_Rig.LoadNearDepth`): while a hand carries a round that a load point would take (`LoadWhy` empty), a faint pulse every 12 tics within twice that point's oval, and a firm one every 5 tics inside it. Every load point on every card gets it.<br>• **Netplay:** the spin count and its gate run in DoEffect and the weapon's own states on every machine, off the owner's usercmd; no RNG, never the console player. The buzz is local presentation, for the console player's own hands only, and decides nothing. |
| 43 | Model-card INHERITANCE, `base = <card>` (the owner, 2026-09-15: "yes do inheritance" -- twin guns differ in skin, handling sounds, hand, prop or reload while sharing one measured card) | **COMPILED** 773.42 ms on exe 09-15 03:14, check only, NOT installed. Not yet played; NO card uses `base` yet, so every gun loads exactly as before.<br>• **Grammar** (parser.zs INHERITANCE): `base = <id>` starts a card from a FRESH COPY of that card (read again from its own lump, so no part or live state is shared), with its own lines laid over it.<br>&nbsp;&nbsp;– Every key it states replaces the base's.<br>&nbsp;&nbsp;– A part, store, verb or barrel block under a base id REPLACES that one whole, in its place; a new id is added.<br>&nbsp;&nbsp;– `remove part|store|verb|barrel <id>` (after `base`, between blocks) takes one away.<br>&nbsp;&nbsp;– The base's synthesised stores are dropped and synthesised again for the whole card.<br>&nbsp;&nbsp;– A base may have a base, up to MAX_BASE_DEPTH 8. A base that cannot be built leaves the child unloaded, said with its line; the moving-surface limit is checked on the whole card.<br>• **Where:** `WM_Parser.ResolveBases` in `WM_System.LoadCards`, after every WMCARD lump and before the sheets, keeping each card's place in the order (CardForAmmo, Equip). `WM_Parser.FreshCard` builds a card whole; `BlockBody` takes a card's own lines with their line numbers kept. `ParseAll` gained an `overlayOnto` card.<br>&nbsp;&nbsp;– `sheet.zs BorrowModels` now takes its copies from FreshCard, so a gun borrowing a child card gets it whole.<br>&nbsp;&nbsp;– `wm_card` prints ", starting from <base>".<br>&nbsp;&nbsp;– `WM_CardSet.basesRead` re-reads a set saved before this.<br>• **Proof still owed** (needs a game run): after install, `wm_card check` empty and `wm_card all` unchanged. Then a first real child card, suggested to the weapons lane: WM_PlasmaRifleBlue as `base = WM_PlasmaRifle` plus its hand, prop and skin, printing the same parts and verbs as today. card_lint must learn `base` / `remove` (the weapons lane's tool).<br>• **Netplay:** load-time data, read alike on every machine; no RNG, no consoleplayer. |
| 44 | The volumetric muzzle flash rides the gun hand (RS_Ballistics df0c301's handoff; the build lane's desk list, 2026-09-15) | **WRITTEN, not compiled** (the owner is playing; no checks until the batch). `WM_Rig.FlashAt` passes `pmo.PlayerNumber()` and the rig's `hand` (0 main, 1 off -- RSB_Flash.Fire's own numbering, checked against RS_Ballistics' flash.zs) as its new optional `shooterPlayer`, `shooterHand`, so an engine #15 volume follows the hand instead of hanging where the shot left.<br>• **Netplay:** looks only; RSB_Flash is client-side. |
| 45 | The MAGFAMILY must-state sweep (the owner's card fix b, 2026-09-15) | **WRITTEN, not compiled.**<br>• **Scan:** 28 of 35 cards stated a family. Seven defaulted to "pistol": six take nothing from a hand (Chaingun, Rotary Gun, Rotary Launcher: reserve; the three saws: none), so it never mattered for them. **WM_MachineGun was a real bug:** its underbarrel grenade load inherited "pistol", so a loose pistol round could load into the launcher.<br>• **Card:** RS_VR_Weapons WMCARD.09 `magfamily = "40mm"` on WM_MachineGun (card_lint 0 issues). The weapons lane commits it.<br>• **Parser:** FinishCard step 3g. A card that takes a magazine or a round from a hand (`WM_Card.TakesFromHand`: a swap, a load, a detachable counted store) with no magfamily is WARNED while `WM_Parser.MAGFAMILY_REQUIRED` is 0, and REFUSED once it is flipped to 1 (a number: a ZScript constant cannot be a bool -- the first batch check refused `= false`). Flip it after the RS_VR_Weapons pack carrying the Machine Gun line is installed, so no Vanilla gun drops out between installs. Doc: parser.zs header and card.zs magFamily.<br>• **Proof owed** (a game run): no 3g warning with the new RS_VR_Weapons pack; a pistol round refused at the Machine Gun's launcher breech ("that round is pistol -- the WM_MachineGun takes 40mm").<br>• **Netplay:** load-time data, alike on every machine. |
...
| 47 | The CARD CHECKER, mod side: WM_CardValidator on the engine's `-validatedata` (build 5, main 98606da42d, exe 09-15 14:13; the owner: "card checker is important") | **COMPILED and INSTALLED 2026-09-15 14:21:38** (811.67 ms, the build's RS_Ballistics-only check with the validators on).<br>• **`WM_Parser.BuildCardSet`:** LoadCards' pipeline is one static function now: every WMCARD lump, ResolveBases, every WMSHEET lump, BorrowModels, ApplyToCards, Finish. `WM_System.LoadCards` and the validator both call it. One visible change: "N weapon sheet(s)" prints after Finish's refusals.<br>• **`zscript/wm/cardvalidator.zs` `WM_CardValidator : DataValidator`:** runs BuildCardSet in the compile check and prints one summary line.<br>• **The two refusal funnels** (`WM_Parser.Refuse`, `WM_SheetReader.Refuse`) also call `DataValidation.Refuse` while `DataValidation.Running()`. That is false in play.<br>• **PROOF, tools/compile_check.ps1 on exe 14:13:** the owner's full 15-file load order COMPILED with no DATA REFUSED (1141 ms). The same order plus a scratch pk3 of two broken cards FAILED with `DATA REFUSED WMCARD line 8 -- card 'thisisnotakey': unknown key` and `DATA REFUSED WMCARD line 3 -- card 'base WM_NoSuchCardForTheDataCheck': that base card could not be built`. The unknown key prints twice because base resolution re-reads that lump; cosmetic.<br>• **Netplay:** runs only inside the compile check.<br>• **MODEL NAMES, INSTALLED 2026-09-15 15:28:00 (build lane's ask: a bad joint name must surface in the checker, not only in the bind log).**<br>&nbsp;&nbsp;– The validator reads each card's own model file (`WM_ModelNames`: IQM mesh and joint names, MD3 surface names). It refuses, at the card line, a part `surface` / `joint` or a card `hidesurface` / `hidejoint` the model does not have, and lists what the model does have.<br>&nbsp;&nbsp;– Names match without case, as the engine's FName lookups do.<br>&nbsp;&nbsp;– Model index 0 only.<br>&nbsp;&nbsp;– A model that is absent or unreadable only prints a note.<br>&nbsp;&nbsp;– `WM_Card.hideSurfaceLines` / `hideJointLines` carry the lines.<br>• **PROOF (staged pk3, owner's 15-file order):**<br>&nbsp;&nbsp;– Today: 52 cards; 182 names in 31 model files; 0 missing.<br>&nbsp;&nbsp;– Good scratch cards (case-changed names, IQM joints and hides): +11 names, 0 missing.<br>&nbsp;&nbsp;– Broken scratch cards: exactly the 4 planted names refused, exit 1339.<br>• **MODEL N (engine build 10, e7e05508ed: the model-index queries), 2026-09-15 16:15.**<br>&nbsp;&nbsp;– A part on `model = N` is checked against the prop class's own MODELDEF model N (`Actor.GetClassModelFile` / `GetClassModelSurfaceName` / `GetClassModelJointName`), the model the rig finds in play. A model N the class does not load only prints a note.<br>&nbsp;&nbsp;– The rig finds joint parts with `FindModelJointIndex(part.modelIndex, name)`. `FindBoneIndex` asked model 0 only and aborts on a non-decoupled actor, which every WM_Prop is. The missing-name logs list the part's own model.<br>• **PROOF (staged pk3s: the owner's 15 files plus RS_Modern after RS_VR_Weapons):**<br>&nbsp;&nbsp;– 53 cards, the Breach Glock included; 186 names; 0 missing.<br>&nbsp;&nbsp;– Scratch parts on model 1 with the right names: +7, 0 missing.<br>&nbsp;&nbsp;– The same parts misspelled: exactly 2 refused, exit 1339. |
| 48 | VANILLA+ ROUND 1 ALT FIRES and the round spread cone (the owner via the weapons lane, 2026-09-15: "JUST PUT THE SHIT ON THE CARDS"; `RS_VR_Weapons/_pending/VANILLA_PLUS_ROUND1.md` section 3; braced / two-handed shelved by the owner) | **COMPILED and INSTALLED 2026-09-15 14:32:34** (1146.10 ms, exe 14:13). Not yet played. The first check refused `WM_Gun.AltModeWord` called from the sheet's data context; it is `clearscope` now.<br>• **Sheet keys** (sheet.zs: fields, SheetKey, the printout Row, Check's Diff, header): `altmode = burst \| selectfire \| shred \| slamfire \| onebarrel \| doubleshell \| fan`, `altburst` (3), `altbursttics` (4), `altratescale` (2), `altdamagescale` (2), `altfanmax` (8), `spreadshape = box \| cone`. WM_Gun properties of the same names; TakeSheet; ShotText.<br>• **`WM_Ready` returns a State.** On a gun whose card puts no barrel on the second button (a barrel wins) and with no switch pending for this hand:<br>&nbsp;&nbsp;– burst: a press sets `burstShotsLeft`;<br>&nbsp;&nbsp;– shred: held, `altShotKind` SHRED;<br>&nbsp;&nbsp;– onebarrel / doubleshell: a press sets the shot kind;<br>&nbsp;&nbsp;– slamfire: `slamPending` fires while alt is held;<br>&nbsp;&nbsp;– fan: `fanPending` fires while the trigger is held.<br>• **`WM_TryFire`:**<br>&nbsp;&nbsp;– ONEBARREL: at most one chamber.<br>&nbsp;&nbsp;– DOUBLESHELL: when CanFire allows 2 chambers / twice the rounds, one shot of PelletsPerShot, each RSB_Bullet's set damage × AltDamageScale (`ScaleShotDamage`; profile damage can't be scaled, warned once); one loaded chamber fires plainly.<br>&nbsp;&nbsp;– Select fire's burst mode arms `burstShotsLeft` on a pull's first shot; every burst round decrements it.<br>&nbsp;&nbsp;– A fanned round has 2× spread.<br>&nbsp;&nbsp;– A dry shot ends a burst.<br>• **`WM_FireWait`:** AltBurstTics between burst rounds, FireTics / AltRateScale for shred, 35 / AltFanMax for a fanned round.<br>• **`WM_HoldFire`:** a burst continues regardless of the trigger; shred continues while alt is held; full auto is `FiresFullAuto()` (select fire's mode 2, else the class's FullAuto).<br>• **`DoEffect`:** select fire steps on the second button's press edge (`StepSelectFire`: single → burst → auto; click and holder's buzz).<br>• **`ScatterShot`:** `spreadshape = cone` draws evenly over a disc of the larger spread; box draws in the same RNG order as before.<br>• **Fan and slamfire, network events:**<br>&nbsp;&nbsp;– `WM_Rig.FanGesture`, every tic for this machine's hands: the other hand ARRIVING within max(grabradius, 4) of the card's `role = hammer` part (grab point, or dof pivot) at ≥ 40 map units/s, trigger held, sends `wm_fan`.<br>&nbsp;&nbsp;– `WM_Rig.StrokeHome` with alt held sends `wm_slam`.<br>&nbsp;&nbsp;– `WM_System.NetworkProcess` hands both to the sender's gun on every machine (`OnFanEvent` capped at AltFanMax, `OnSlamEvent`).<br>• **Netplay:** every shot decision is the weapon's own state off the owner's usercmd, or an owner-sent network event; the named RNG WMSpread; the haptic only for the console player.<br>• **Follow-up, COMPILED and INSTALLED 14:44:48 (797.60 ms, exe 14:37): doubleshell's second shell from the pump tube.** The weapons lane found that the Quad Super (WM_BullpupPump's card: one chamber slot, a counted tube) always fell back to a plain shot. When a chamber gun's chamber holds only one live round, doubleshell now takes its second shell from the store its cycle verb feeds from (`WM_System.DoubleShellFeed`, `WM_Ammo.SpendFromStore`), spent with the shot. With an empty tube it is the plain shot. No card change. |
| 49 | THE GUN SIDE OF THE BONE DRIVE: card parts that are JOINTS (Engine docs/MODEL_JOINT_DRIVE_PLAN.md section 4; engine pieces D, A, B and C in builds 6, 7 and 9, main a3db6c8dfa) | **COMPILED and INSTALLED 2026-09-15 15:11:21** (900.96 ms, exe 15:08), applied from `_staged/JOINT_PARTS_MOD_HUNKS.py` (24 hunks). No card uses it yet; the first IQM gun card (Breach) waits for the owner.<br>• **Card keys:** part `joint = <name>` (card.zs `WM_Part.jointName`); card `hidesurface = <mesh>` and `hidejoint = <joint>`, both repeatable (`WM_Card.hideSurfaces` / `hideJoints`). The verb and latch checks accept a joint part.<br>• **`WM_Rig`:**<br>&nbsp;&nbsp;– a joint part resolves with `FindBoneIndex`, and a missing joint is logged at bind;<br>&nbsp;&nbsp;– `PoseJointPart` each tic: `MJP_Hide` / `MJP_Clear` for hidden or absent, else `SetModelJointOffset` from the same `PartOffset` / `PartRotation` a surface part gets;<br>&nbsp;&nbsp;– `StartJointDrive`: `SetModelJointDrive` / `...Rotation` / `...Hinge` / `...Stage` on StartDrive's own axes, signs and pivots;<br>&nbsp;&nbsp;– `StopDrive` reads `GetModelJointDrawnValue` and switches the drive off (entry kept);<br>&nbsp;&nbsp;– `DrawnValue`, Reset, the split log and the magazine drop know `jointDriven`.<br>• **Gun-wide hides:** override slots after the parts' (`hideSlotBase`), resolved in Resolve and re-asserted in Pose.<br>• **`WM_System`:** unbind clears `jointDriven`; grab candidates take a joint part.<br>• **The Body IK lane's note, kept:** a joint's offset and drive entries are set once and updated; no grab adds or removes one.<br>• **Not yet:** a hand seat rides the whole gun, not the part (piece E); roundsurface / metersurface / flip are surface-only.<br>• **Netplay:** as surface parts -- local presentation off the local controller; the drawn value has GetModelSurfaceDrawnValue's status (NETWORK_HAND_INPUT_PLAN.md 4.4). |
